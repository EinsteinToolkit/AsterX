...
namespace BNS_READER {
    BNS_READER::reader_t* bns_reader{nullptr};

    void malloc_bns_reader(std::string & fn) {
        bns_reader = new BNS_READER::reader_t(fn);
    }
    void free_bns_reader() {
        delete bns_reader;
    }
    extern "C" void KadathImporterBNS(CCTK_ARGUMENTS) { 
        DECLARE_CCTK_ARGUMENTS;
        DECLARE_CCTK_PARAMETERS;
        using namespace Loop;
        
        using importer_t = BNS_READER::reader_t;
        static std::mutex copy_mutex;
        // Copy ID object per thread (hopefully)        
        copy_mutex.lock();
        importer_t importer(*bns_reader);
        copy_mutex.unlock();
